...
flowchart TD
    subgraph "Design Decisions"
        A["Array vs Pointer-based"]
        B["Generic vs Concrete Types"]
        C["Panic vs Result Types"]
        D["1-based vs 0-based Indexing"]
    end
    
    subgraph "Chosen Solutions"
        E["Array: Better cache locality"]
        F["Generic: Reusable for any monoid"]
        G["Panic: Simpler API, clear contracts"]
        H["1-based: Cleaner parent-child math"]
    end
    
    subgraph "Benefits"
        I["10-30% performance improvement"]
        J["Type safety + code reuse"]
        K["Easier debugging + clear failures"]
        L["No off-by-one errors"]
    end
